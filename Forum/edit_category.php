...
{
$id = intval($_GET['id']);
$dn1 = mysqli_fetch_array(mysqli_query('select count(id) as nb1, name, description from categories where id="'.$id.'" group by id'));
if($dn1['nb1']>0)
{
...
if(isset($_POST['name'], $_POST['description']) and $_POST['name']!='')
{
	...
	if(mysqli_query('update categories set name="'.$name.'", description="'.$description.'" where id="'.$id.'"'))
	{
	?>
