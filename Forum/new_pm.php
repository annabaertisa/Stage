...
if(isset($_POST['title'], $_POST['recip'], $_POST['message']))
{
	...
	if($_POST['title']!='' and $_POST['recip']!='' and $_POST['message']!='')
	{
		...
		$dn1 = mysqli_fetch_array(mysqli_query('select count(id) as recip, id as recipid, (select count(*) from pm) as npm from users where username="'.$recip.'"'));
		if($dn1['recip']==1)
		{
			if($dn1['recipid']!=$_SESSION['userid'])
			{
				$id = $dn1['npm']+1;
				if(mysqli_query('insert into pm (id, id2, title, user1, user2, message, timestamp, user1read, user2read)values("'.$id.'", "1", "'.$title.'", "'.$_SESSION['userid'].'", "'.$dn1['recipid'].'", "'.$message.'", "'.time().'", "yes", "no")'))
				{
	?>
